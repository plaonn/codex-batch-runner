...
def run_cli(args: list[str], *, cwd: Path | None = None) -> tuple[int, dict]:
    stdout = io.StringIO()
    cwd_context = chdir(cwd) if cwd else contextlib.nullcontext()
    with cwd_context, contextlib.redirect_stdout(stdout):
        code = main(args)
...
def write_config(root: Path, repo: Path, *, config_root: Path | None = None) -> Path:
    data = {
        "root": str(config_root or repo),
...
class DirectWorktreeMaintenanceTests(unittest.TestCase):
    def test_dry_run_defaults_to_current_git_root_with_central_runtime_config(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            repo.mkdir()
            init_repo(repo)
            config_path = write_config(runtime_root, repo, config_root=runtime_root)

            create_direct_worktree(repo, root / "demo-merged-clean", "codex/merged-clean", commit=True, merge=True, dirty=False)

            code, report = run_cli(
                ["--config", str(config_path), "maintenance", "direct-worktrees", "--dry-run", "--json"],
                cwd=repo,
            )

            self.assertEqual(0, code)
            self.assertEqual(str(repo.resolve()), report["repo_root"])
            self.assertEqual(["codex/merged-clean"], [candidate["branch"] for candidate in report["eligible"]])

    def test_repo_root_option_targets_repo_even_when_cwd_is_another_repo(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            runtime_root = root / "runtime"
            ...
            config_path = write_config(runtime_root, target_repo, config_root=runtime_root)

            create_direct_worktree(target_repo, root / "demo-merged-clean", "codex/merged-clean", commit=True, merge=True, dirty=False)
            ...
            self.assertEqual(0, code)
            self.assertEqual(str(target_repo.resolve()), report["repo_root"])
            self.assertEqual(["codex/merged-clean"], [candidate["branch"] for candidate in report["eligible"]])
