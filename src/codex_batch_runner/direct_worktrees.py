...
def build_direct_worktrees_report(config: Config, *, apply: bool = False, repo_root: Path | None = None) -> dict[str, Any]:
    if apply:
        ...
        try:
            return _build_direct_worktrees_report_locked(config, apply=True, repo_root=repo_root)
        finally:
            lock.release()
    return _build_direct_worktrees_report_locked(config, apply=False, repo_root=repo_root)


def _build_direct_worktrees_report_locked(config: Config, *, apply: bool, repo_root: Path | None) -> dict[str, Any]:
    report = base_direct_report(apply=apply)
    try:
        target_repo_root = resolve_target_repo_root(repo_root)
        target = git_optional(target_repo_root, "symbolic-ref", "--quiet", "--short", "HEAD") or "HEAD"
        target_head = git(target_repo_root, "rev-parse", "--verify", "HEAD^{commit}")
        task_root = config.worktree_root.expanduser().resolve() if config.worktree_root else None
        entries = worktree_registry(target_repo_root)
    ...
    report.update(
        {
            ...
            "allowlist": {
                ...
                "path_pattern": f"../{target_repo_root.name}-*",
            },
            "planned_action": "git worktree remove <path>; git branch -d <branch>",
        }
    )
    candidates = [
        classify_direct_worktree_entry(config, target_repo_root, task_root, target, entry)
        for entry in entries
        if should_inspect_entry(target_repo_root, task_root, entry)
    ]
    ...
    for candidate in report["eligible"]:
        refreshed = refreshed_candidate(config, target_repo_root, task_root, target, candidate)
        if not refreshed.get("eligible"):
            result = apply_result_from_refreshed(refreshed, status="blocked")
            results.append(result)
            continue
        result = remove_direct_worktree(config, target_repo_root, target, refreshed)
        results.append(result)
...
def resolve_target_repo_root(repo_root: Path | None) -> Path:
    start = repo_root.expanduser() if repo_root else Path.cwd()
