...
향후 notification, Telegram adapter, dashboard, optional SQLite index를 위한 event log roadmap도 [docs/spec.md](docs/spec.md)에 정리되어 있습니다.
...
기본 `list` 출력은 운영자가 봐야 할 남은 작업 중심의 compact human list입니다. `completed + accepted`, `archived`, resolution이 기록된 `failed`/`blocked_user` task는 기본 출력에서 숨기고, 전체 이력이 필요하면 `--all`을 사용합니다. 첫 줄은 `ID`, `STATUS`, `PROJECT`, `ATT`, `DEPS`, `NOTE`를 표시하고, 각 task의 다음 줄에는 `TITLE`을 표시합니다. `ID`는 복사하기 쉽게 그대로 표시하며, `DEPS`는 dependency title이 아니라 concise task id를 쉼표로 표시하고 dependency가 없으면 `-`를 표시합니다. `NOTE`는 cooldown, dependency blocked 상태, review 상태, resolution, failed error, startup stall evidence, running runtime/progress 같은 운영 정보를 사람이 읽는 문장으로 표시하고 없으면 `-`를 표시합니다. 스크립트에서는 사람이 읽는 list 대신 `--json` 출력을 사용해야 합니다.
...
Human 출력은 `--color=auto|always|never`를 지원합니다. 기본값 `auto`는 TTY에서만 색을 사용하고 `NO_COLOR`가 설정되어 있으면 색을 끕니다. `always`는 강제로 색을 켜고, `never`는 항상 끕니다. 같은 task id는 stable color를 받으며 `DEPS`에 같은 id가 나타날 때도 같은 색을 사용합니다. 상태 색은 주의가 필요한 `failed`, `blocked_user`, `review_failed`, `needs_followup`은 red, 검토 대기/진행은 yellow, 실행 중은 cyan, 실행 가능/재개 대기는 blue, cooldown/usage exhausted 계열은 dim, completed/accepted는 `--all`에서만 green 계열로 표시합니다. `--json` 출력에는 ANSI color code를 넣지 않습니다.
