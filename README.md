...
관련 프로젝트에서 배치 결과를 점검할 때는 `show`, `transcript`, `logs`, 필요한 테스트 명령을 함께 확인한 뒤 `accept` 또는 `reject`를 사용합니다.

여러 프로젝트가 하나의 중앙 queue를 공유하는 운영을 위해 project metadata와 review 대상 필터를 추가할 계획입니다. 목표는 관련 프로젝트에서 자기 작업만 저비용으로 고른 뒤, 필요한 task에 대해서만 `show`나 `transcript`를 읽는 흐름입니다. 세부 설계는 [docs/spec.md](docs/spec.md)의 project routing metadata와 operational triage plan을 기준으로 관리합니다.
