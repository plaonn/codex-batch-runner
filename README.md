...
`list --graph`는 기본 compact list와 별도의 human dependency graph를 출력합니다. Project section 아래에 source task를 `* status title` graph node로 표시하고, dependency가 있으면 source graph에는 `|\`와 `|/` wrapper를 남기되 dependency 자체는 graph rail에 연결하지 않고 더 들여쓴 `|-- dependency-state title`, `` `-- dependency-state title`` child line으로 표시합니다. Graph mode는 사람이 dependency shape를 직관적으로 보는 view이므로 task id, attempts, note는 출력하지 않습니다. Source task status는 기본 list와 같은 effective status와 color policy를 사용하고, dependency state label은 기본 `DEPS` 열과 같은 readiness policy와 color policy를 사용합니다. Color-enabled 출력에서는 dependency title을 항상 dim 처리해 source task와 시각적 위계를 둡니다. 좁은 terminal에서는 graph/tree prefix와 status/state label 영역을 유지하고 title만 continuation line으로 wrap합니다. `--graph`는 human renderer만 바꾸며 `--json`을 함께 쓰면 기존과 같은 raw task JSON 배열을 출력합니다. `--watch --graph`는 같은 graph renderer를 반복 갱신합니다.
