...
기본 `list` 출력은 운영자가 봐야 할 남은 작업 중심의 compact human list입니다. `completed + accepted`, `archived`, resolution이 기록된 `failed`/`blocked_user` task는 기본 출력에서 숨기고, 전체 이력이 필요하면 `--all`을 사용합니다. 첫 줄은 `PROJECT`, `TITLE(ID)`, `STATUS`, `ATT`, `DEPS`, `NOTE`를 표시합니다. 각 task는 최소 두 줄로 표시하며 첫 줄의 `TITLE(ID)`에는 사람이 읽는 title을, 다음 줄에는 `(task-id)`를 표시합니다. `DEPS`는 dependency title이 아니라 task id를 표시하며, dependency가 여러 개이면 한 줄에 하나씩 이어지는 continuation row에 세로로 표시합니다. Dependency가 없으면 첫 줄에 `-`를 표시합니다. `NOTE`는 cooldown, dependency blocked 상태, review 상태, resolution, failed error, startup stall evidence, running runtime/progress 같은 운영 정보를 사람이 읽는 문장으로 표시하고 없으면 `-`를 표시합니다. 스크립트에서는 사람이 읽는 list 대신 `--json` 출력을 사용해야 합니다.
...
자동 검토는 runner와 같은 queue lock을 사용하는 순차 phase로만 동작합니다. 기본값은 report-only이고, `"auto_review_mechanical_accept": true` 또는 `review-next --apply --mechanical-auto-accept`처럼 명시적으로 허용한 경우에만 local mechanical gate를 모두 통과한 task를 자동 accept합니다. Gate가 실패하거나 상태가 stale이면 human fallback으로 남기며, reviewer Codex는 기본적으로 비활성화되어 token을 소비하지 않습니다. 자세한 설계는 [docs/spec.md](docs/spec.md)의 automatic review bundle section을 기준으로 합니다.
