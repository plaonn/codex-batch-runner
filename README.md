...
Optional `post_mutation_trigger_command` can run a generic wake-up hook after successful queue mutations. The value is an argv list, not a shell string, so it is not shell-expanded. It is disabled by default.
...
The hook runs after durable task state writes for commands such as `enqueue`, `accept`, `reject`, `resolve`, and `archive`. It is not run by read-only commands, `run-next`, or `prune`. Hook failures print a warning to stderr but do not fail the core mutation. Polling remains the fallback, and duplicate wake-ups are safe because `run-next` still enforces the runner lock, cooldown checks, empty-queue behavior, and single-task execution.
...
Queue mutation 직후 launchd job을 깨우려면 config에 generic post-mutation hook을 추가할 수 있습니다. label은 로컬 환경에 맞게 바꿉니다.
...
향후 자동 검토는 rule-only auto-accept가 아니라 mechanical gates, self-contained review bundle, reviewer Codex, human fallback 단계로 나눕니다. 현재 구현은 `cbr review-bundle TASK_ID`와 `cbr review-next --dry-run` 같은 report-only 흐름으로 시작하며, 자세한 설계는 [docs/spec.md](docs/spec.md)의 automatic review bundle section을 기준으로 합니다.
