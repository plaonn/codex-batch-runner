...
- `worktree reconciliation-plan`은 같은 owner projection과 canonical result/review/apply/cleanup metadata를 snapshot digest에 exact-bind하고 current/grandfathered row를 `no_action`, `manual_review`, `exact_repair_candidate`, `unrecoverable_without_owner_decision` 중 하나로 report-only 분류합니다. Exact candidate는 독립적으로 입증된 applied terminal cleanup의 `execution_worktree_status` enum-only correction만 표시하며 repair authority나 apply surface를 제공하지 않습니다. Missing path만으로 hibernated/cleaned/disposition/deletion eligibility를 추론하지 않습니다.
