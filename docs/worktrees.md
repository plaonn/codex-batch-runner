...
- `cbr doctor`는 branch lifecycle visibility를 read-only로 제공합니다. JSON `worktree.task_branches`와 human `task_branches`는 retained/cleaned/pruned task branch metadata, local branch existence/head, apply/cleanup/prune status, path existence boolean, and locally known/configured remote task branch refs를 표시합니다. 이 check는 local git metadata만 읽으며 branch deletion, remote pruning, fetch, pull, or push를 수행하지 않습니다.
