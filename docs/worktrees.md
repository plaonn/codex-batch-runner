...
- `worktree hibernation-plan`은 task metadata, Git worktree registry, branch/head를 read-only로 reconciliation하고 branch-only review와 future hibernation/reattach/resume compatibility를 서로 다른 projection으로 표시합니다. Missing path는 hibernation state가 아니며, v1에는 apply나 lifecycle mutation이 없습니다.
