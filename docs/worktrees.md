...
- `worktree hibernation-plan`은 task metadata, Git worktree registry, branch/head를 read-only로 reconciliation하고 branch-only review와 hibernation/reattach/resume compatibility를 서로 다른 projection으로 표시합니다. Missing path는 hibernation state가 아니며 plan 자체에는 apply나 lifecycle mutation이 없습니다.
- `worktree reconciliation-plan`은 같은 owner projection과 canonical result/review/apply/cleanup/branch-prune/conflict-fix/pool metadata를 snapshot digest에 exact-bind하고 current/grandfathered row를 `no_action`, `manual_review`, `exact_repair_candidate`, `unrecoverable_without_owner_decision` 중 하나로 report-only 분류합니다. Exact candidate는 독립적으로 입증된 applied terminal cleanup의 `execution_worktree_status` enum-only correction만 표시하며 pooled row는 canonical task/branch/slot/policy와 local pool-state observation이 일치하는 released lease receipt까지 요구합니다. Pooled cleanup이 보존한 idle slot directory/registry entry는 observed slot path와 exact match하고 task branch registry entry가 없을 때만 current입니다. Valid cleanup 뒤 branch-prune와 conflict-fix apply-via terminal state는 owner receipt가 exact하면 `no_action`입니다. Repair authority나 apply surface는 없고 missing path만으로 hibernated/cleaned/disposition/deletion eligibility를 추론하지 않습니다.
- `worktree reconciliation-repair TASK_ID --approved-source-digest DIGEST [--apply]`는 별도 single-task C2 surface입니다. Default dry-run이며 apply는 queue lock 아래에서 live C1 item과 exact task CAS를 다시 검증한 뒤 allowlisted `retained|recovery_required -> cleaned` task field만 atomic update합니다. Prepared/committed sanitized audit phase가 partial-write recovery와 duplicate no-op을 bind하며 worktree/branch/registry/pool/result/review/resolution/archive는 변경하지 않습니다.
- `worktree hibernate TASK_ID --dry-run|--apply`는 queue lock 아래에서 완료·clean·checkpoint·branch-only-review·mutation-provenance·pool-lease gate를 다시 확인한 단일 task만 명시적으로 hibernate합니다. Disposable worktree는 exact path만 제거하고 pooled worktree는 exact lease만 해제하며 branch/base/checkpoint와 result/review 상태는 보존합니다.
- `worktree reattach TASK_ID --dry-run|--apply`는 `worktree-hibernation-v1` intent가 기록된 `hibernated` task만 받습니다. Branch/base/checkpoint, 다른 checkout 부재, pooled preparation-policy fingerprint를 확인한 뒤 새 disposable path 또는 compatible pool slot에 연결하며 worker를 실행하지 않습니다. Git/pool mutation 이후 metadata 저장이 끝나지 않으면 `recovery_required`로 fail closed합니다.
- Hibernation/reattach는 bulk, 자동 hook, TTL, GC, branch prune/delete, migration/repair, missing-path inference, `needs_resume` cwd 재생성을 제공하지 않습니다.
