...
- [Gateway-neutral execution plan](gateway-neutral-execution-plan.md): exact-target/delegation-bound read-only launch projection, public-safe policy metadata, deterministic unavailable reasons, and strict no-launch/no-mutation boundary.
...
- [Execution-target selector decision envelope](execution-target-selector-decision-envelope.md): strict report-only task/attempt binding for canonical override or authoritative absence over the unchanged ordering-v1 baseline.
