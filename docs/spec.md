...
- `routing_size`: pre-enqueue work size estimate. 허용값은 `tiny`, `small`, `medium`, `large`, `xlarge`입니다. 이 값은 audit metadata이며 `small` 값이 `execution_profile=small`을 의미하지 않습니다.
...
이 field들은 task selection, model selection, profile fallback을 자동 변경하지 않습니다. 실제 profile 선택은 task `execution_profile`, config의 default/review profile 및 high-risk fallback, task-level Codex override에 의해 결정됩니다. Prompt text, raw runtime state, session/thread id, credential, private local path를 넣지 않는 공개 가능한 짧은 metadata로만 사용합니다.
...
`cbr routing-report`는 profile selection을 조정하기 위한 read-only evidence surface입니다. 명령은 queue task를 profile, resolved profile, category, label, profile/category 조합, routing experiment, routing size, routing risk, routing risk factor, verification scope, routing decision tuple, profile/routing decision tuple, resolved-profile/routing decision tuple, small-profile candidate signal, profile/experiment 조합으로 집계하고 accepted count, first-pass accepted count, needs-fix/rejected rate, reviewer decision count, auto-fix task frequency, attempts, run count, duration 기반 cost proxy를 출력합니다. 기존 `profile` field와 group은 JSON compatibility를 위해 task의 explicit `execution_profile`을 사용하고 없으면 `default`로 묶습니다. `resolved_profile`은 `last_run.execution_profile`이 있으면 그 값을 쓰고, 없으면 explicit `execution_profile`, 둘 다 없으면 `default`를 씁니다. Routing decision tuple은 `routing_size`, `routing_risk`, sorted `verification_scope`를 `size=... risk=... verify=...` 형태로 묶은 key이고, profile/routing decision tuple과 resolved-profile/routing decision tuple은 여기에 각 profile view를 붙인 key입니다. `small_profile_candidate`는 `routing_size`가 `tiny` 또는 `small`, `routing_risk=low`, `verification_scope`가 `docs`/`none`으로 제한되고 configured/resolved profile이 모두 `small`이 아닐 때 `candidate`로 표시합니다. JSON report에는 prompt 없이 task id, profile/resolved_profile/category/label, routing metadata, routing decision key, candidate signal, outcome/cost summary만 담은 `task_rows`도 포함합니다. Report는 task JSON, event log, review status를 변경하지 않고 Codex 또는 reviewer Codex를 호출하지 않습니다. 운영자는 이 결과를 보고 상위 profile로 올릴 high-risk pattern 또는 하위 profile로 내릴 low-risk pattern을 별도 policy change로 반영합니다.
...
- `small` 또는 동등한 저비용 profile은 bounded, low-blast-radius 작업에서만 명시적으로 사용합니다. 예시는 공개 문서의 작은 수정, 예제/README 보강, 테스트 fixture 또는 단순 textual cleanup처럼 실패해도 리뷰 단계에서 쉽게 감지되고 main apply 전 되돌릴 수 있는 작업입니다. `routing_size=small`은 이런 검토를 돕는 audit metadata일 뿐이며, 실제 small 실행은 `execution_profile=small` 또는 그에 해당하는 config policy가 선택해야 합니다.
...
- `cbr routing-report --json` 또는 human report에서 `by_routing_decision`은 같은 size/risk/verification 요구가 전반적으로 안정적인지 확인하는 기준이고, `by_profile_routing_decision`은 explicit task profile 기준 outcome/cost를, `by_resolved_profile_routing_decision`은 실제 recorded runtime profile 기준 outcome/cost를 확인하는 기준입니다. `by_small_profile_candidate`는 conservative low-risk docs/none tuple이 `small` profile로 실행되지 않은 후보를 찾는 보조 신호입니다.
...
`cbr list` human 출력은 optional color를 지원합니다. `--color=auto|always|never` 중 하나를 사용할 수 있으며 기본값은 `auto`입니다. `auto`는 stdout이 TTY이고 `NO_COLOR`가 없을 때만 색을 켭니다. `always`는 색을 강제로 켜고 `never`는 항상 끕니다. `DEPS` blocker label은 dependency task의 effective status style을 사용합니다. 만족된 dependency는 color-enabled 출력에서 dim style로 표시해 inactive dependency임을 구분합니다. `STATUS`는 color-enabled human output에서 foreground 색만이 아니라 background가 있는 label 형태로 표시합니다. 상태 label 색은 문제 또는 후속 조치가 필요한 `failed`, `blocked_user`, `blocked_dependency`, `review_failed`, `needs_followup`은 red, 검토 대기/진행은 yellow, 실행 중은 cyan, 실행 가능/재개 대기는 blue, cooldown/usage exhausted 계열은 dim, completed/accepted는 전체 이력 조회에서만 green 계열로 표시합니다. 색은 보조 시각 정보이며 색이 꺼져도 같은 정보를 텍스트로 읽을 수 있어야 합니다. `--json` 출력에는 ANSI code를 포함하지 않습니다.
