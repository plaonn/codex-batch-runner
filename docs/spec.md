...
`cbr list --dependency-graph`는 기본 compact list와 분리된 human dependency edge list입니다. 각 row는 `TASK`가 `WAITS_FOR`에 표시된 dependency를 기다린다는 edge를 나타냅니다. 출력 열은 `PROJECT`, `TASK`, `STATUS`, `WAITS_FOR`, `DEP_STATE`, `TASK_TITLE`, `DEP_TITLE`입니다. 이 모드는 project-task-subtask tree connector를 쓰지 않으며, task tree와 dependency DAG를 같은 ASCII line system에 섞지 않습니다. Dependency가 없는 task는 `WAITS_FOR=-`, `DEP_STATE=none`으로 표시합니다. `DEP_STATE`는 `done`, `blocked`, `not_accepted`, `not_applied`, `missing`을 사용하며 기본 compact `DEPS` 열과 같은 dependency readiness policy를 적용합니다. `--dependency-graph`는 human renderer만 바꾸고, `--json`과 함께 사용하면 graph-specific JSON schema를 만들지 않고 기존 raw task JSON 배열을 그대로 출력합니다. `--watch --dependency-graph`는 같은 graph renderer를 반복 갱신합니다.
