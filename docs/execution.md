...
`cbr provider-resource-report`는 위 routing evaluation marker와도 분리된 `provider-resource-snapshot-v1` 및 `provider-resource-mapping-v1` evidence surface입니다. Local capacity와 provider resource를 나란히 표시하지만 병합하지 않습니다. D1 output은 non-authoritative이며 defer/wake/cooldown 생성, alternative target 선택, global usage admission mutation, v1/v2/v3 execution evidence 변경을 수행할 수 없습니다. Unknown, stale, invalid, unavailable, missing, ambiguous resource evidence는 report에 표시하고 새 resource-aware candidate flag에서만 제외하며 기존 execution은 바뀌지 않습니다. 상세 계약은 [provider resource report](provider-resource-report.md)를 참조합니다.
