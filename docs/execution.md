...
이 문서는 execution profile, shell backend, capacity/priority, runner admission, Codex command wrapper, watchdog, lock, atomic write, rate-limit, queue mutation control plane을 정의합니다. 핵심 스펙 index는 [spec.md](spec.md)입니다.
...
이 field들은 task selection을 변경하지 않지만, bounded low-risk 작업을 `small` profile로 자동 downshift할 때 profile fallback 입력으로 사용합니다. 실제 profile 선택은 task `execution_profile`, config의 default/review profile, high-risk fallback, low-risk small fallback, task-level Codex override에 의해 결정됩니다. Prompt text, raw runtime state, session/thread id, credential, private local path를 넣지 않는 공개 가능한 짧은 metadata로만 사용합니다.
...
Low-risk routing metadata에는 보수적 `small` fallback을 적용합니다. Task가 명시적으로 `execution_profile`을 지정하지 않았고 `default_execution_profile`이 설정되어 있으며 config에 `small` profile이 있고, high-risk fallback에 걸리지 않으며, `routing_size`가 `tiny` 또는 `small`, `routing_risk=low`, `verification_scope`가 비어 있거나 `none`/`docs`로만 구성되어 있으면 runner는 `small` profile을 사용합니다. Unit, integration, build, e2e처럼 검증 범위가 넓은 작업은 이 fallback 대상이 아닙니다.
...
- 실제 profile 기준 변경은 report 실행과 분리된 operator change로 수행합니다. 대상 tuple, 기존 profile, 새 profile, 근거 report 범위, rollback 기준을 public-safe docs 또는 local operator memo에 남긴 뒤 execution fallback, enqueue helper 기준, 또는 config를 수정합니다.
