...
- `capacity_target_ordering_canary_policy`: default-disabled exact-scope canary policy. 초기 enabled assignment는 정확히 5%이고 hard ceiling은 10%로 고정되며, kill switch와 allowed scope가 claim-time baseline reordering을 fail-closed로 제한합니다. 상세 계약은 [bounded capacity target-ordering canary mechanism](capacity-target-ordering-canary.md)에 정의됩니다.
...
`cbr provider-resource-report`는 위 routing evaluation marker와도 분리된 provider resource evidence surface입니다. V1 snapshot/mapping advisory compatibility를 유지하며, mapping v2와 admission policy v1을 제공하면 strict authority eligibility를 read-only preview합니다. Local capacity와 provider resource를 나란히 표시하지만 병합하지 않습니다. D1/D1.6 output은 non-authoritative이며 defer/wake/cooldown 생성, alternative target 선택, global usage admission mutation, v1/v2/v3 execution evidence 변경을 수행할 수 없습니다. D2-A `cbr provider-resource-simulate`는 selected exact target과 hard-constraint/quality-floor를 다시 통과한 alternative exact target에 대해 `allow|defer|covered_by_global|evidence_only` 결정, reset grace wake time, canonical resource/decision/wake key만 preview합니다. Global terminal gate가 먼저 적용되고 같은 reset은 `covered_by_global`로 dedup됩니다. Unknown, stale, invalid, unavailable, missing, ambiguous resource evidence는 report에 표시하고 기존 execution을 유지하며 절대 `defer`를 만들지 않습니다. 두 command 모두 queue, cooldown, wake, target substitution, routing policy, config, execution evidence를 변경하지 않으며 D2-B activation은 금지됩니다. Typed gate decision/state와 stable dedup keys도 runtime scaffold일 뿐 state에 저장되지 않습니다. 상세 계약은 [provider resource report](provider-resource-report.md)를 참조합니다.
...
Report 기본 `--purpose routing`은 cohort metadata상 `model_quality=true`인 comparable row만
포함함. Legacy/non-comparable run을 quota/debugging과 운영 진단에 볼 때는
`--purpose diagnostic`을 명시함. `--purpose audit`은 archived task까지 포함해 retained
