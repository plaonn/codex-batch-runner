...
- `cbr policy-proposals execution-target-freshness`는 freshness state를 read-only proposal JSON으로 표시합니다. `cbr policy-proposals preview PROPOSAL_JSON`은 이 report를 read-only preview로 렌더링합니다. 현재 proposal class는 execution target freshness만 다루며 apply/config rewrite/task mutation/model replacement/rule replacement를 수행하지 않습니다.
