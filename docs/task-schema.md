...
- `execution_mutation_snapshot_history`: isolated task worktree의 pre-worker, post-worker/pre-CBR-commit, terminal closure observation을 task attempt별 append-only로 보존합니다. Raw path 대신 repository/worktree identity, Git status, task/review state의 deterministic digest와 count만 기록합니다.
- `execution_mutation_provenance_history`: `cbr-execution-mutation-provenance-v1` scoped record의 append-only 목록입니다. External service/account/network/arbitrary filesystem은 scope 밖이며 global provenance는 항상 `unknown`입니다. 자세한 계약은 [Scoped execution mutation provenance](execution-mutation-provenance.md)를 참고합니다.
- `execution_delegation_contract`: optional `cbr-execution-delegation-contract-v1`. First atomic enqueue write에서만 admit되며 이후 immutable합니다. Post-hoc insertion과 historical backfill은 거부됩니다.
- `preexecution_delegation_receipt_history`: exact worker/target resolution 뒤, pre-worker snapshot 및 worker invocation 전에 runner가 canonical append하는 `cbr-preexecution-delegation-receipt-v1` 목록입니다.
- `preexecution_delegation_phase_history`: receipt durable append, pre-worker snapshot binding 또는 receipt-only stale recovery 순서를 append-only로 보존합니다. 자세한 계약은 [Trusted pre-execution delegation](execution-delegation.md)을 참고합니다.
