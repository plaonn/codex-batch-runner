...
Generic `cbr prune`은 age/review/archive 조건을 만족한 task의 raw runtime log와 cursor-safe
event file만 삭제할 수 있음. Canonical task JSON은 legacy/debug/audit evidence를 포함하므로
별도 versioned deletion/cold-storage policy가 생기기 전까지 항상 blocked/skipped로
보고하고 물리 삭제하지 않음. 따라서 `prune`은 canonical evidence deletion 명령이 아님.
