...
- Revisit signal status: D1 planner, D2 explicit CBR dispatch, D3-0 generic reconciliation shadow, D3-1 CBR-owned local ingress plus durable shadow state, and D3-2 explicit one-event guarded consumer with bounded lease/retry and local immutable disposition implemented; automatic discovery, external disposition delivery, coordination mutation, and same-call execution remain unauthorized.
