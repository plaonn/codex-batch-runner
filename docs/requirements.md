...
- Derived specs: task JSON은 requirement vector를 저장하고, planned selection은 resolved execution/worker config에 기록하며, observed actual model/usage는 source/confidence/availability가 있는 evaluation evidence v2로 별도 저장한다. Review outcome evidence는 별도 append-only history로 acceptance method, objective verification, semantic review, reviewer provenance를 보존하고 서로 다른 acceptance method를 aggregate하지 않는다. Cohort는 evidence/policy/selection/worker/reviewer/derivation version boundary와 comparability를 보존하며, review quality comparison은 task bucket, execution cohort, outcome contract, review policy, rubric, acceptance method, reviewer provenance class가 같은 strata로 제한한다. Legacy evidence는 v2 actual-model cohort나 review quality/anchor coverage 분모·분자에 합치지 않는다. External actual-model/usage attestation은 optional capability이며 생략 시 unavailable로 남긴다. Routing/provider evidence report는 read-only/advisory 진단으로만 사용하고, queue 밖 실행 근거는 fake task가 아니라 execution surface가 구분된 supplemental evidence row로 병합한다.
