...
Config는 `queue_dir`, `log_dir`, `lock_file`, `state_file`, Codex command
template, stale lock 기준, rate-limit cooldown, 기본 max attempts를 제어합니다.
상대 runtime path는 `cbr` process의 current working directory를 기준으로
해석됩니다. 자동화에서는 config에 absolute path를 쓰거나 안정적인
`WorkingDirectory`를 지정해야 합니다.
...
`launchd`, cron, script, Codex skill, 여러 working directory에서 실행될 수 있는
workflow에서는 `--config`를 명시합니다. User config discovery는 운영자 개인의
interactive shell에는 편리하지만 unattended automation을 디버깅할 때 원인을
찾기 어렵게 만들 수 있습니다. Built-in fallback은 process working directory의
`.codex-batch-runner/` 아래에 runtime state를 만들므로 빠른 local 실험에는
충분하지만 shared beta queue에는 적합하지 않습니다.
