...
PYTHONPATH=src python3 -m codex_batch_runner --config /path/to/cbr-config.json run-loop --json
```
...
- `ProgramArguments`: module invocation 또는 installed `cbr` absolute path, 보통 `run-loop --json`
...
```json
{
  ...
  "usage_admission_primary_threshold_percent": 10,
  "usage_admission_secondary_threshold_percent": 5,
...
`usage_admission_max_age_seconds` 이내의 fresh snapshot이 threshold 이하이면 reset + grace까지
global cooldown이 설정됩니다. Secondary threshold는 optional이며, configured되어 secondary가
낮으면 secondary reset이 필요하고 그 시각까지 gate합니다. Primary와 secondary가 모두 낮으면
두 reset 중 나중 시각을 선택합니다. Triggering window reset이 없거나 invalid이면 unrelated
