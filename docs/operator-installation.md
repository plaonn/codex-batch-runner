...
Interactive Bash/Zsh Tab completion만 optional `completion` extra를 사용합니다.
기본 설치와 scheduler runtime에는 필요하지 않습니다.
...
Shell completion을 사용할 때만 extra를 설치하고, 출력된 등록 script를 현재
shell에서 명시적으로 평가합니다. `cbr`는 shell rc 파일이나 completion directory를
자동 수정하지 않습니다.

```bash
/absolute/path/to/python3.11 -m pip install -e '.[completion]'
eval "$(cbr completion zsh)"
# 또는
eval "$(cbr completion bash)"
```

Completion은 command, nested subcommand, option, choice, file/directory path를
parser에서 생성합니다. Task ID 후보를 조회할 때는 일반 config discovery 순서를
사용하지만 queue/config/state를 생성하거나 수정하지 않습니다. Config나 queue를
읽을 수 없으면 정적 completion은 유지하고 task 후보만 조용히 비웁니다.
...
`bootout`은 destination plist path를 다시 해석하지 않고 exact `gui/UID/LABEL` service target을
사용합니다. `bootstrap`은 public `launchctl` interface가 요구하는 검증된 absolute plist path를
사용하며 호출 직전에 held directory/snapshot identity를 다시 확인합니다. `/dev/fd` bootstrap은
공개 계약이 아니므로 사용하지 않습니다.

이 guard가 지원하는 threat model은 accidental/non-adversarial namespace concurrency입니다.
동시 변경을 발견하면 작업을 중단하거나 관찰된 identity를 다시 확인한 뒤 rollback을
best-effort로 시도합니다. 동일 UID가 마지막 검사 이후 namespace를 적극적으로 계속 바꾸는
adversarial mutation은 public rename/launchctl interface만으로 원자적으로 차단할 수 없어 지원하지
않습니다. 이 위협까지 다뤄야 하는 deployment는 LaunchAgents namespace를 보호하거나 privileged
helper 경계를 별도로 도입해야 합니다. Human/JSON 결과의 `namespace_concurrency_*`/
`namespace_concurrency` 필드가 이 경계를 항상 표시합니다.
