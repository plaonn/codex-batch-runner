...
올리면 여러 scheduler worker 또는 겹친 `run-next` 호출이 서로 다른 admissible
task를 동시에 claim할 수 있습니다. cbr는 in-process dispatcher를 시작하지
...
기본 `cbr list`는 compact human 표기로 `[P]`, `TITLE`, `STATUS`, `ATT`, `DEPS`, `NOTE`
열을 보여 줍니다. `FLAGS`라는 고정 열은 없고, 의사결정에 필요한 상태는 `DEPS`/`NOTE`/`STATUS`에 합쳐져 표시됩니다.
의존성 구조를 별도 트리 형태로 보고 싶으면 `--graph`를 사용합니다.
