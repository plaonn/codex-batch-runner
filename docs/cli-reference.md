...
`cbr status [--json]`는 Codex나 운영 자동화가 cbr를 실행 표면으로 쓸지 판단하기 위한 lightweight read-only preflight입니다. `state`처럼 raw runner state만 출력하지 않고, runner pause, global cooldown, reviewer Codex cooldown, active lock, queue admissible count, review backlog, accepted-but-unapplied worktree count를 한 report로 요약합니다. JSON의 `admission.status`는 `available`, `paused`, `cooldown`, `locked` 중 하나이며, `can_enqueue`, `can_run_next`, `can_start_cbr_execution`, `blocked_reasons`, `recommended_action`을 함께 제공합니다. 이 명령은 runner를 실행하지 않고, lock을 획득하지 않고, queue/state/event 파일이나 디렉터리를 생성하지 않으며, post-mutation trigger를 실행하지 않습니다.
...
`cbr execution-report [--project PROJECT_ID] [--project-root PATH] [--category CATEGORY] [--label LABEL] [--limit N] [--include-archived] [--json]`는 enqueue 시점의 의도 대신 실제 처리된 task run만 요약하는 read-only report입니다. 각 row는 task title, status/review status, started/finished time, queue wait, duration, execution backend, capacity pool, worker family, return code, timeout 여부, model source/selection rule/execution target, resolved worker target/model group, external worker `--model-group`, result count, token usage를 포함합니다. Codex run의 token usage는 configured `log_dir` 내부 attempt JSONL에서 최신 `usage` object를 읽어 `input_tokens`, `cached_input_tokens`, `uncached_input_tokens`, `output_tokens`, `reasoning_output_tokens`, `known_total_tokens`로 표시합니다. Shell backend는 `token_usage_source=token_free`이고, external-json-command/Antigravity처럼 usage가 노출되지 않는 backend는 `token_usage_source=unavailable`입니다. 이 report는 raw prompt, transcript/log body, full command argv, session/thread id, raw log path를 출력하지 않고 task JSON/event log/review status를 변경하지 않습니다.

`cbr routing-policy-candidates [--project PROJECT_ID] [--category CATEGORY] [--label LABEL] [--execution-evidence-json PATH] [--include-non-reviewable] [--json]`는 `routing-report`의 existing evaluation diagnostics task bucket advisory를 policy-change candidate surface로 재표시합니다. 기본 출력은 `threshold_advisory_status=reviewable` bucket만 `candidates`로 내보내며 각 entry는 `candidate_id`, `task_bucket_key`, evidence counts/rates, advisory status/reasons, fixed thresholds, `recommended_next_step=operator_review`, `read_only=true`, `mutation_allowed=false`를 포함합니다. `decision_cards`는 실행 후보 보고 상태와 사용자 결정 상태를 별도 축으로 표시하며, reviewable bucket은 `user_decision_status=decision_required`, non-reviewable bucket은 `user_decision_status=not_ready`로 표시합니다. `--include-non-reviewable`은 `insufficient_sample`/`below_threshold` bucket을 `non_reviewable_buckets`와 observation decision card에 함께 표시하고 `blocked_reason`, `rejection_reasons`, next step을 드러내며, human output은 recommendation/blocker summary groups를 표 위에 표시합니다. 이 명령은 candidate를 승인하지 않고, model/routing/provider config를 쓰지 않으며, task JSON/event log/review status를 변경하지 않습니다.

`cbr decision-cards [--project PROJECT_ID] [--category CATEGORY] [--label LABEL] [--execution-evidence-json PATH] [--include-observations] [--source SOURCE] [--decision-axis AXIS] [--user-decision-status STATUS] [--json]`는 현재 config의 `policy-proposals execution-target-freshness` decision cards와 `routing-policy-candidates` decision cards를 하나의 read-only inventory로 모읍니다. 기본 출력은 `decision_required`와 `approval_blocked`처럼 사용자가 볼 필요가 있는 카드만 포함하고, `--include-observations`를 붙이면 routing `not_ready` 관찰 카드도 함께 포함합니다. `--source`는 `policy-proposals execution-target-freshness` 또는 `routing-policy-candidates`를 받는 repeatable allowlist filter입니다. `--decision-axis`는 `execution_target_freshness` 또는 `routing_policy_change`를 받는 repeatable allowlist filter입니다. `--user-decision-status`는 repeatable allowlist filter이며 `decision_required`, `approval_blocked`, `decision_pending`, `approved`, `not_approved`, `not_ready`, `invalid` 중 하나를 받습니다. JSON에는 inventory `generated_at`, 각 source report의 generated timestamp/read-only metadata, `summary.by_source`, `summary.by_recommendation`, `summary.by_blocked_reason`, `summary.next_action`이 포함됩니다. `summary.next_action`은 카드가 없거나 모든 카드가 terminal `approved`/`not_approved` 상태이면 `none`, `invalid` 카드가 있으면 `fix_invalid_decision_cards`, actionable `decision_required`/`approval_blocked`/`decision_pending` 카드가 있으면 `review_decision_cards`, 그 외 `not_ready` 관찰 카드가 있으면 `continue_observing`입니다. Human `open_decisions`는 `summary.next_action`이 `fix_invalid_decision_cards` 또는 `review_decision_cards`일 때만 `present`입니다. Human output도 recommendation/blocker summary groups, per-card `BLOCKED`, next action hint를 표시합니다. 이 명령은 하위 report builder를 읽기 전용으로 호출하며 approval, apply, model/routing/provider config write, task/event/runtime mutation을 수행하지 않습니다.

`cbr watching-report [--project PROJECT_ID] [--category CATEGORY] [--label LABEL] [--execution-evidence-json PATH] [--include-archived] [--json]`는 queue, review/apply, worktree lifecycle, cooldown/rate-limit, routing policy observation evidence를 read-only area report로 모읍니다. 각 area는 `evidence_status`를 `action_required`, `ready_for_close_review`, `continue_observing`, `no_evidence` 중 하나로 표시하고, operator-facing `operator_next_action`과 compact signal list를 제공합니다. `summary.next_action`은 action-required area가 있으면 `resolve_action_required`, 닫을지 검토할 evidence가 있으면 `review_close_candidates`, 아직 부족하면 `continue_observing`, 아무 evidence가 없으면 `none`입니다. 이 report는 operator의 external watching/task dashboard를 자동으로 변경하지 않으며, task JSON, event log, review status, policy/config를 쓰지 않습니다.
...
`cbr doctor`는 저비용 health check임. resolved `queue_dir`, `log_dir`, `event_dir`, `lock_file`, `state_file` 경로, runtime directory 접근 가능 여부, configured Codex executable path, resolved Codex executable path, executable availability, bounded configured executable `--version` output, model selection rule 이름과 allowlisted override key, execution target freshness 상태와 stale warning, decision-card count/source/recommendation/blocker summary, decision-card next action hint, global cooldown, runner pause, active lock age/pid/liveness, needs-review completed task 수, runnable task 수, cooldown task 수, startup/no-progress stall evidence, resolved failed/blocked task 수, resolved completed-review task 수, status별 task 수, mechanical auto-review enable 상태, reviewable completed task 수를 표시함. Human `tasks`는 operator-actionable count를 먼저 표시한 뒤 raw `by_status` inventory를 표시하고, Human `checks`는 ok/warning/error count와 warning/error detail 중심으로 요약하고, capacity는 running/admissible/blocked/over-capacity summary와 compact pool summary 중심으로 표시하며, 전체 check/capacity detail은 `--json` output에 유지합니다. Version 확인은 configured executable에 `--version`만 붙여 짧은 timeout으로 실행하며, `codex exec`를 호출하지 않음. Version command 실패, 빈 output, timeout은 warning으로 보고하고 doctor 실패로 취급하지 않으며, warning message는 wrapper가 설정된 경우에도 실제 configured executable 이름을 기준으로 표시합니다. configured/current project root가 git repository 안에 있으면 branch, dirty status, upstream 또는 local `origin/main` 대비 ahead/behind count도 표시함. `worktree.task_branches` JSON section과 human `task_branches` subsection은 task worktree branch lifecycle을 read-only로 표시합니다. 여기에는 task id, local branch name/existence/head, retained/cleaned/pruned metadata, apply/cleanup/prune status, path existence boolean, and locally known/configured remote task branch refs가 포함됩니다. Human output은 task branch total/displayed/omitted count를 표시하고 상세 행은 처음 20개로 제한하며, JSON output은 전체 `worktree.task_branches`를 유지합니다. git check는 local repository metadata만 읽고 fetch/pull/push 같은 network operation을 실행하지 않음. git executable 없음, git repository 아님, upstream 없음, remote ref 조회 불가 같은 상태는 warning으로 보고하고 doctor 실패로 취급하지 않음. `--json`을 붙이면 같은 정보를 JSON으로 출력함. error check가 있으면 non-zero로 종료하고 warning은 종료 코드에 영향을 주지 않음.
