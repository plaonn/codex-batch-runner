...
`cbr maintenance direct-worktrees`는 cbr task metadata가 없는 direct operator git worktree를 audit하거나 보수적으로 정리합니다. Discovery는 현재 configured root repository에서 `git worktree list --porcelain`로 등록된 worktree만 사용하며, filesystem glob scan은 하지 않습니다. Main worktree와 configured `worktree_root` 아래 task worktree는 제외합니다.
